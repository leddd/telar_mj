...
class KeyStroke:
    ...
    def update(self, frame, surface):
        ...
        if pct < 0.25:
            alpha = int(255 * (pct / 0.25))
        elif pct > 0.75:
            alpha = int(255 * ((1 - pct) / 0.25))
        else:
            alpha = 255
        ...
        for i in range(len(self.original_curves)):
            ...
            points = [cubic_bezier(*animated, t / 20.0) for t in range(21)]
            for k in range(len(points) - 1):
                pygame.draw.aaline(curve_surface, stroke_color, points[k], points[k + 1])

        surface.blit(curve_surface, (0, 0))
